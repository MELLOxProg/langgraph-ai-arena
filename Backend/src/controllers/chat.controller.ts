...
export async function sendMessage(req: Request, res: Response) {
  ...
  let chat = chatId
    ? await chatModel.findOne({ _id: chatId, user: req.user!.id })
    : null;
  if (chatId && !chat)
  ...
  const context = history
    .map(
      (entry) =>
        `${entry.role === "user" ? "User" : "Arena response"}: ${entry.content}`,
    )
    .join("\n\n");
