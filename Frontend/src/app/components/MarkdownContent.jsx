/**
 * Lightweight markdown renderer using Tailwind classes via the
 * `md-content` @layer component defined in index.css.
 *
  ...
  switch (block.type) {
    ...
    case 'code': return (
      <div key={i} className="md-code-block">
        ...
        <pre className="md-pre"><code>{block.text}</code></pre>
      </div>
    )
    case 'table': return (
      <div key={i} className="md-table-wrapper">
        <table className="md-table">
          {block.header && (
            <thead>
              <tr>
                {block.header.map((cell, j) => <th key={j} className="md-th">{inlineRender(cell)}</th>)}
              </tr>
            </thead>
          )}
          <tbody>
            {block.rows.map((row, j) => (
              <tr key={j} className="md-tr">
                {row.map((cell, k) => <td key={k} className="md-td">{inlineRender(cell)}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
    ...
    case 'ul': return (
      <ul key={i} className="md-ul">
        {block.items.map((item, j) => <li key={j} className="md-li">{inlineRender(item)}</li>)}
      </ul>
    )
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('|')) {
      ...
      while (i < lines.length && lines[i].startsWith('|')) {
        ...
        if (lines[i].includes('---')) { /* separator */ } else if (!header) { header = cells } else { rows.push(cells) }
        i++
      }
    ...
    while (i < lines.length && lines[i].trim() !== '' && !lines[i].startsWith('#') && !lines[i].startsWith('```') && !lines[i].startsWith('|') && !/^[-*] /.test(lines[i]) && !/^---+$/.test(lines[i].trim())) {
      paragraphLines.push(lines[i]); i++
    }
    if (paragraphLines.length > 0) blocks.push({ type: 'p', text: paragraphLines.join(' ') })
  }
  return blocks
}
